...
import { CreateProduct, DeleteProductParam, GetProductParam, UpdatProduct, UpdateProductParam } from "../../interfaces/product.interface";
...
export const ProductRoutes = async (app: FastifyInstance) => {
  ...
  app.get('/product/:restaurantId', async (request: FastifyRequest<{ Params: GetProductParam }>, response: FastifyReply) => {
    const restaurantId = request.params.restaurantId
    const result = await productService.get(restaurantId);
    return response.status(200).send(result)
  })

  app.put('/product/:id', async (request: FastifyRequest<{ Params: UpdateProductParam, Body: UpdatProduct }>, response: FastifyReply) => {
  ...
  app.delete('/product/:id', async (request: FastifyRequest<{ Params: DeleteProductParam }>, response: FastifyReply) => {
    const { id } = request.params
